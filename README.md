...
- キャラZIP読込時の、チャットパレット内にある定義形式の `/buff` 行からのバフ/デバフテンプレート自動登録
- BCDice 風の四則演算を使ったリソース操作コマンド
- ラウンド変更時のメインチャット通知と、各チャットへのラウンド注釈
- 全カテゴリ検索・図解付きのタブ切り替え式ヘルプページ
...
GMモード中は、秘匿中のステータスやGM向けチャット内容を確認できます。GMモード中に追加したキャラクターも、詳細表示やチャットパレットは通常どおり開けます。

GMモードは各参加者が自分で切り替えられる表示モードで、アクセス権限を制御する機能ではありません。
...
ステータス秘匿は表示を整理する機能で、キャラクター詳細の閲覧そのものを禁止する機能ではありません。
...
ラウンド1以上の間は、各キャラクターのチャットパレットまたはキャラコマの右クリックメニューから行動完了を切り替えられます。盤面上の名前横には行動状態アイコンが表示され、未行動はグレー、完了済みはチェックマークになります。行動完了時はメインチャットにもログが流れます。ラウンドが0の時は操作とアイコンを表示しません。ラウンドが1以上の時は、テーブル上の全キャラクターが行動完了になるまで次ラウンドへ進めません。

キャラクターごとにバフ/デバフテンプレートを登録でき、UIまたはチャットコマンドから実行できます。`/buff キャッツアイ :MP-3` のように、バフ付与コマンドやテンプレートの末尾へリソース操作を付けて、MP消費なども同時に処理できます。

キャラZIPには、付与中の効果と効果管理画面で作成したテンプレートは保存されません。テンプレートも持ち運ぶ場合は、効果管理の「全テンプレートをコピー」で得た `/buff 効果名/効果/ラウンド` 形式の定義行をチャットパレットへ登録してから保存してください。キャラZIP読込時に、その定義行からテンプレートが自動登録されます。`/buff キャッツアイ :MP-3` のような呼び出しだけの行は登録対象になりません。
...
最大値を超えないようにする `M` と、計算結果による増減方向の逆転を0に抑える `!` も利用できます。
