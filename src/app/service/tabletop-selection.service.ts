...
export class TabletopSelectionService {
  static instance: TabletopSelectionService = null;
  ...
  constructor() {
    TabletopSelectionService.instance = this;
  }
  ...
  toggle(object: TabletopObject, state: SelectionState = SelectionState.SELECTED) {
    if (this.selectionMap.has(object)) {
      this.remove(object);
    } else {
      this.add(object, state);
    }
  }
