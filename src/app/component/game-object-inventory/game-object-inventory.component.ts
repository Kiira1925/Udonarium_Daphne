...
export class GameObjectInventoryComponent implements OnInit, OnDestroy {
  ...
  isStatusHidden(gameObject: GameObject): boolean {
    return gameObject instanceof GameCharacter && gameObject.isStatusHidden && !RoomState.instance.isGM();
  }
  ...
  onContextMenu(e: Event, gameObject: GameCharacter) {
    ...
    if (this.canAccessGameObject(gameObject)) {
      actions.push({ name: '詳細を表示', action: () => { this.showDetail(gameObject); } });
    }
    if (this.canAccessGameObject(gameObject) && gameObject.location.name !== 'graveyard') {
      actions.push({ name: 'チャットパレットを表示', action: () => { this.showChatPalette(gameObject) } });
      actions.push({ name: '効果管理を開く', action: () => { this.showEffectManagement(gameObject) } });
    }
  ...
  private cloneGameObject(gameObject: TabletopObject) {
    ...
    if (cloneObject instanceof GameCharacter) {
      cloneObject.markAsCreatedBy(Network.peer.userId, RoomState.instance.isGM() || gameObject instanceof GameCharacter && gameObject.isGMCreated);
    }
  }
  ...
  private canAccessGameObject(gameObject: GameObject): boolean {
    return !(gameObject instanceof GameCharacter) || RoomState.instance.canAccessGMCharacter(gameObject);
  }
