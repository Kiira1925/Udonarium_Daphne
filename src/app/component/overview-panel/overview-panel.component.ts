...
export class OverviewPanelComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  get isCharacterStatusHidden(): boolean {
    return this.tabletopObject instanceof GameCharacter && this.tabletopObject.isStatusHidden && !RoomState.instance.isGM();
  }
  ...
  displayValue(dataElement: DataElement): string | number {
    return this.isCharacterStatusHidden ? '??' : dataElement.value;
  }

  displayCurrentValue(dataElement: DataElement): string | number {
    return this.isCharacterStatusHidden ? '??' : dataElement.currentValue;
  }
