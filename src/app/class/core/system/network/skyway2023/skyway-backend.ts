...
async function fetchStatus(url: string): Promise<boolean> {
  try {
    let api = backendApiUrl(url, '/v1/status');
...
async function fetchSkyWayAuthToken(url: string, channelName: string, peerId: string): Promise<string> {
  try {
    let api = backendApiUrl(url, '/v1/skyway2023/token');
...
function backendApiUrl(url: string, pathname: string): URL {
  let baseUrl = url || location.origin;
  if (baseUrl.includes('{your-backend-hostname}')) baseUrl = location.origin;
